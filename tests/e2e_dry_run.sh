...
assert_contains "$out" "DRY  cd /tmp && codex resume" "codex resume line (canonical; no cod alias in test shell)"
assert_contains "$out" "DRY  cd /tmp && claude --resume" "claude resume line (canonical; no cc alias in test shell)"
