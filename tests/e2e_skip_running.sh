...
out="$("$PFR" --dry-run "${FIX_ARGS[@]}" --state-dir "$SD" --ps-file "$PSF" --no-save-plan 2>&1)" \
...
out="$("$PFR" --dry-run "${FIX_ARGS[@]}" --state-dir "$SD" --ps-file "$PSF" --force-reopen --no-save-plan 2>&1)" \
